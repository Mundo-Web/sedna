...
import SolutionSubRest from "../actions/Admin/SolutionSubRest";
...
import { add } from "sode-extend-react/sources/notify";

const servicesRest = new SolutionSubRest();
...
const SolutionSubtheme = ({ solutions }) => {
    ...
    const onModalOpen = (data) => {
        ...
        if (data?.partners?.length) {
            setPartners(
                data.partners.map((ben) => ({
                    title: ben.title,
                    image: ben.image,
                }))
            );
        } else {
            setPartners([{ title: "", image: undefined }]);
        }
    ...
    const onModalSubmit = async (e) => {
        ...
        partners.forEach((char, index) => {
            formData.append(`partners[${index}][title]`, char.title);
            if (char.image) {
                if (char.image.file) {
                    formData.append(
        ...
        formData.append("solution_id", selectedSolution);
    ...
    return (
        <>
                    ...
                    },
                    {
                        ...
                        cellTemplate: (container, { data }) => {
                            ReactAppend(
                                container,
                                <img
                                    src={`/api/solutionSubtheme/media/${data.image}`}
                                    style={{
                                        width: "80px",
                                        height: "45px",
                                        objectFit: "cover",
            ...
            <Modal
                modalRef={modalRef}
                title={isEditing ? "Editar SubSolución" : "Nueva SubSolución"}
