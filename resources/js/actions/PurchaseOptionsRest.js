...
class PurchaseOptionsRest extends BasicRest {
    path = "purchaseOptions";
    
    getOptions = async (params = {}) => {  // Añade parámetros opcionales
        try {
            const { status, result } = await Fetch(
                `/api/${this.path}/getOptions`,
                {
                    method: "POST",
...
export default PurchaseOptionsRest;
